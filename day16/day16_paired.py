...
def comparePaths(paths, z, flowDict, newGraph, maxScores, num_cores):
    tempScore = 0
    for i in range(z, z + num_cores) if z + num_cores < len(paths) - 1 else range(z, len(paths) - 1):
...
def part2():
    ...
    with open('input.txt') as f:
        ...
        for z in range(0, len(paths) - 1, partitionSize):
            pool.apply_async(comparePaths, args=(paths, z, flowDict, newGraph, maxScores, partitionSize))
